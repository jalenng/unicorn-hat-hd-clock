...
def draw_time():
    ...
    # Draw hour digits
    hour_x_offset = 0 if TWELVE_HR_FORMAT else -1 
    if not (OMIT_LEADING_ZERO and int(hour_str[0]) == 0):
        draw_frame(cache['digits'][int(hour_str[0])], -1 + hour_x_offset, 11, filters)
    draw_frame(cache['digits'][int(hour_str[1])], 3 + hour_x_offset, 11, filters)
...
def draw_weather():
    ...
    draw_frame(weather_sprite[frame_index], 3, 0)


def draw_blinker():
    if TWELVE_HR_FORMAT:
        show = get_time().second % 2 == 0
        filter_to_use = filters if show else off_filters
        draw_frame(cache['digits'][10], 6, 11, filter_to_use)
